...
<%--Save Credit Card--%>
<script type="text/javascript">
	function CheckValidUsZipCode() {
            var raw_number = $('#txtCCZip').val();
           /* var regex1 = /^[0-9]{5}(?:-[0-9]{4})?$/;*/
            if (raw_number.length > 5 && raw_number.length <5) 
			{              
				$("#dvCreditCardAlertDiv").html('<div class="alert alert-error">Enter correct zip code</div>');                
                return false;
            }
			 else {
       ...
       // $("#dvSaveOrderBtn").hide();
        if ($('#dvEditPaymentMethod').css('display') == "block") {
			...
			if (validate.error == undefined) {
				$('#dvCreditCardAlertDiv').html('');
				validate.checkMinLength('txtCCZip', 5, 'zip code must be 5 characters.');
				}
       else {
           $('#dvCreditCardAlertDiv').html('');
       }
    ...
    <div class="osPointTotal">
        ...
        <div class="osPointRight">
            ...
            <div class="osCCTextBoxRow" id="dvEditPaymentMethod" style="display: none;">
                ...
                <div class="osCCZipCol" style="display:none;">
