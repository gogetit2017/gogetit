...
backgroundImage: '',
		...
		showLoading: function () {
			...
			$('<div id="fancybox-loading"></div>').click(F.cancel).appendTo('form');
		},
		...
		_afterLoad: function () {
			...
			F.isOpen = false;
			F.current = F.coming;
			F.current.backgroundImage = F.coming.backgroundImage;
			F.coming = false;

			//Build the neccessary markup
			F.wrap = $(F.current.tpl.wrap).addClass('fancybox-tmp ' + F.current.wrapCSS).appendTo('form');
		...
		_setContent: function () {
			...
			switch (type) {
				...
				case 'html':
					...
					if (current.autoSize) {
						loadingBay = $('<div class="fancybox-tmp"></div>').appendTo($("form")).append(content);
...
$(outer).css('background-image','url('+backgroundImage+')');
			//Reset dimensions
			$(wrap.add(outer).add(inner)).width('auto').height('auto');

			inner.width(width - padding2).height('auto');
			wrap.width(width);
